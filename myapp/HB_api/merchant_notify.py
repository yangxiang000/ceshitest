...
def merchant_notify(request):
    ...
    if not verification:
        expected_dict = {'expectedTypeRange': [dict], 'expectedDict': {
            ...
            "auditResult": {'expectedTypeRange': [str]},
            "auditDescribe": {'expectedTypeRange': [str]}
        }}
        # 验证接口数据格式与数据值是否正确
        if views.is_data_valid(expected_dict, date):
            insret = "insert HB_merchant_notify(" + ",".join(list(date.keys())) + ") values('" + "','".join(
                list(date.values())) + "');"
            print(insret)
            mysql.sql(insret)
